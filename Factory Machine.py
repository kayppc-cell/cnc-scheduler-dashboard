...
# =========================================================
# 5. Scheduling Engine (รันตามลำดับเวลาจริง ไม่แซงคิววัสดุ)
# =========================================================
def calculate_shop_schedule(jobs_df, default_start_datetime):
    ...
    while valid_jobs:
        ...
        # เครื่องใดที่มีคิว 'กำลังผลิต' ค้างอยู่ ให้เครื่องนั้นรันคิวออกไปก่อนเสมอ
        ...
        # ลำดับคิว: 1. กำลังผลิต -> 2. งานด่วนแทรก -> 3. เรียงตามวัน-เวลาขึ้นงาน (ready_at) จากเก่าไปใหม่
        ...
        if running_pool:
            ...
            if urgent_pool:
                # เรียงงานด่วนตามเวลาขึ้นงาน
                urgent_pool.sort(key=lambda x: (x["ready_at"], x.get("ID", 0)))
                selected_job = urgent_pool[0]
            else:
                # เรียงงานปกติตามวัน-เวลาขึ้นงานจริง (ไม่แซงคิววัสดุ)
                ready_candidates.sort(key=lambda x: (x["ready_at"], x.get("ID", 0)))
...
# ---------------------------------------------------------
# VIEW 1: หน้าจอช่างหน้าเครื่อง (เชื่อมโยงตามลำดับ Work Order 100%)
# ---------------------------------------------------------
if st.session_state.current_view == "👷 โหมดช่างหน้าเครื่อง":
    ...
    # 1. รัน Scheduling Engine เพื่อได้ลำดับคิวและเวลาจบงานจริงของเครื่องนี้
    planned_finish_map = {}
    m_wo_order_ids = []
    ...
    if not df_all.empty:
        ...
        if not df_summary_plan.empty:
            for _, s_row in df_summary_plan.iterrows():
                ...
                planned_finish_map[(str(s_row["แผนงาน"]), str(s_row["ชื่อ Drawing."]))] = s_row.get("เวลาจบงาน_DT")
            
            # เก็บ ID ที่เรียงลำดับเวลาเริ่มจริงของเครื่องจักรนี้
            m_summary = df_summary_plan[df_summary_plan["เครื่องจักร"] == selected_m].sort_values(by="เวลาเริ่มจริง", ascending=True)
            m_wo_order_ids = m_summary["ID"].tolist()
    ...
    if not m_all_jobs.empty:
        ...
        for _, g_row in raw_groups.iterrows():
            ...
            if has_pending:
                ...
                # หา Step ปัจจุบันที่ยังไม่เสร็จ
                active_steps_g = steps[~steps["สถานะงาน"].str.contains("เสร็จสิ้น")]
                active_step_id = active_steps_g.iloc[0]["ID"] if not active_steps_g.empty else steps.iloc[0]["ID"]
                
                # ลำดับคิวตาม Work Order (ถ้ามี)
                wo_rank = m_wo_order_ids.index(active_step_id) if active_step_id in m_wo_order_ids else 9999
        ...
        # เรียงลำดับการ์ดคิวหน้าช่าง ให้ตรงกับ Work Order 100%
        sorted_groups = sorted(group_meta, key=lambda x: x["wo_rank"])
